  ...
  return new Promise(async function (resolve, reject) {
    ...
    //            will be in the catch portion.
      try {
        ...
        let URLArray = await fetch(RECIPE_URLS[i]);
        ...
        //            "await" again
        let URLResponse = await URLArray.json();
        // A8. TODO - Add the new recipe to the recipes array
        receiptArray.push(URLResponse);
        ...
        //            resolve() method.
        if (RECIPE_URLS.length == receiptArray.length) {
          saveRecipesToStorage(receiptArray);
